...
#[no_mangle]
pub extern "C" fn libafl_main1() {
    // tree_fuzzer::main(tree_sitter_json::language(), tree_sitter_json::NODE_TYPES); 
    let data = r#"{"name": "John Doe", "age": 30, "vip": true}"#;
    // unsafe {
    //     test_json(data.as_bytes().as_ptr(), data.len());
    // }
    let mut test1 = [1, 2, 3];
    // unsafe {
    //     test1.as_mut_ptr().add(6).write(2);
    // }
    libfuzzer_test_one_input(data.as_bytes());
    unsafe {llvmtt1();}
    println!("Hello, world!");
}
